		...
		for _ in 0..n {
			...
			if !cx.pvfs.is_empty() {
				alternatives.push(|input, cx| {
					...
					for _ in 0..n {
						xs.push(input.int_in_range(0..=cx.pvfs.len() - 1)?);
					}
				...
				alternatives.push(|input, cx| {
					cx.involves_prepare = true;
					let pvf_idx = input.int_in_range(0..=cx.pvfs.len() - 1)?;
					let params = match cx.pvfs[pvf_idx] {
						PvfKind::Adder => {
...
async fn torture() {
	// TODO:
	// there are going to be multiple iterations
	// each iteration we will generate a random script
}
